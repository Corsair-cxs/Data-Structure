...
# 11 "2.third_search.c" 2
# 1 "/usr/lib/gcc/x86_64-linux-gnu/9/include/stdarg.h" 1 3 4
# 12 "2.third_search.c" 2
...
T thrid_search(T head, T tail, int flag, T(*func)(T, int, T*), int n, T* argv) {
    if (abs(tail - head) < 1e-10) return head;
...
int main() {
    ...
    { __typeof(n) __temp = n; printf("Func(x) = "); while (--n > -1) { printf("[ ""%lf"" * (x ^ %d) ]", hz_argv[n], n); n && printf(" + "); } printf("\n"); n = __temp;};
